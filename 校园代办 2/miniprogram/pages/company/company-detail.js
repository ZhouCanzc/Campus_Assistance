// pages/company/company-detail.js
...
Page({
  ...
  onShareAppMessage() {
    return {
      title: '陪替服务 - ' + this.data.orderDetail.title,
      path: '/pages/company/company-detail?id=' + this.data.orderId
    };
  },
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          if (orderManager.acceptOrder('company', order.id, accepterInfo)) {
  ...
  cancelAccept() {
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          if (orderManager.cancelOrder('company', this.data.orderId)) {
            orderManager.notifyUpdate('company');
            this.loadOrderDetail(this.data.orderId);
  ...
  completeOrder() {
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          if (orderManager.completeOrder('company', this.data.orderId)) {
            orderManager.notifyUpdate('company');
            this.loadOrderDetail(this.data.orderId);
