...
Page({
  ...
  loadOrderDetail: function(id) {
    ...
    if (order) {
      ...
      const displayOrder = {
        ...
        pickupDetail: canViewPrivacy ? order.pickupDetail : '**********', // 取件码等敏感信息
        deliveryDetail: canViewPrivacy ? order.deliveryDetail : '**********'
      };
  ...
  onShareAppMessage: function() {
    return {
      title: '帮忙代取快递 - ' + this.data.orderDetail.price + '元',
      path: '/pages/express/express-detail?id=' + this.data.orderId
    };
  },
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          if (orderManager.acceptOrder('express', order.id, accepterInfo)) {
  ...
  cancelOrder: function() {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          if (orderManager.cancelOrder('express', order.id)) {
  ...
  completeOrder: function() {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          if (orderManager.completeOrder('express', order.id)) {
  ...
  goToPayment: function() {
    const order = this.data.orderDetail;
    wx.navigateTo({
      url: `/pages/payment/payment?orderId=${order.id}&amount=${order.price}&title=${encodeURIComponent(order.title || '代取快递')}`
    });
  }
});
